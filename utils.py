...
class Masked_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        x = torch.tensor(self.x[index], dtype=torch.int64).to('cuda')  
        y = torch.tensor(self.y[index], dtype=torch.int64).to('cuda') 
    
        masks = []
        x_masked = x.clone().detach() 
        y_masked = x.clone().detach()  
    
        seq_len = torch.sum(x_masked > 0).item() 
        
        mask = random.sample(self.box[x_masked.cpu() > 0].tolist(), int(seq_len * self.masked_rate))
    
        masks.append(mask)
        no_mask = [b for b in self.box[x_masked.cpu() > 0].tolist() if b not in mask]
